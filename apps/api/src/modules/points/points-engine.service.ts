...
export class PointsEngineService {
   ...
   */
  async handleVisitorBrought(visitId: string, memberId: string) {
    await this.awardPoints(
      memberId,
  ...
  private async calculateAttendanceRate(memberId: string, month: number, year: number) {
    ...
    const attendedEvents = attendances.filter(a => a.status === "ATTENDED").length
  ...
  private async countVisits(memberId: string, startDate: Date, endDate: Date) {
    return this.prisma.memberVisit.count({
      where: {
        memberId: memberId,
  ...
  private async countRecommendations(memberId: string, startDate: Date, endDate: Date) {
    const made = await this.prisma.recommendation.count({
      where: {
    ...
    const closed = await this.prisma.recommendation.count({
      where: {
        recommenderId: memberId,
        status: 'CLOSED_WON',
  ...
  private async updateMemberMetrics(memberId: string) {
    ...
    await this.prisma.memberMetrics.upsert({
      where: {
  ...
  async getMemberPoints(memberId: string) {
    return this.prisma.memberPoints.findUnique({
      where: { memberId },
      include: {
        transactions: {
  ...
  async resetMonthlyPoints() {
    await this.prisma.memberPoints.updateMany({
